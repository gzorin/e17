...
Eina_Bool
engine_rsxgl_args(const char *engine, int width, int height)
{
   Evas_Engine_Info_RSXGL *einfo;

   evas_output_method_set(evas, evas_render_method_lookup("rsxgl"));
   einfo = (Evas_Engine_Info_RSXGL *)evas_engine_info_get(evas);
   if (!einfo)
     {
        printf("Evas does not support the RSXGL Engine\n");
        return EINA_FALSE;
     }

   if (!evas_engine_info_set(evas, (Evas_Engine_Info *)einfo))
     {
        printf("Evas can not setup the informations of the RSXGL Engine\n");
        return EINA_FALSE;
     }
